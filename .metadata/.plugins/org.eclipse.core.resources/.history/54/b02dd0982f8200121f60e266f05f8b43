...
double dRand()
{
	...
	for(int i=0; i<15000; i++)
	{
		of << ((double)rand()/(static_cast<double>(RAND_MAX)+1))*d+a << endl;
 	}
...
double defaultPRNGk()
{
	...
	cout << "Relative error: " << (fabs(calcKmoment-kMoment)/calcKmoment) << endl;
...
double defaultPRNG()
{
 ...
 double monteCarlo5D()
 {
	 ...
	 cout << "Relative error = " << fabs((random5D - 18.75)/18.75) << endl;
	 return 0;
 }
